...
状況、責務を具体的に1件以上書く。
Issueに境界の記載がなければ、推定した境界の根拠を`ASM-XXX`として記録する。
非目標を空にしたり、`なし`、`未定`、`N/A`で済ませたりしない。
spec-generatorはすべての`REQ`と`NFR`にも、観測可能な合否条件を含む空でない
`受入条件：`一覧を置く。
...
6. [ ] 生成または修正後に投影整合パスが完了している
7. [ ] 非目標が空でなく、すべてのREQ/NFRに受入条件がある
