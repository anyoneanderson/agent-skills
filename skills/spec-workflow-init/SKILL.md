...
options:
  ...
  - "Multi-agent (Recommended)" / "マルチエージェント（推奨） — Codex custom agents または Claude Code agent team で役割分担"
  - "Multi-agent (cmux)" / "マルチエージェント(cmux) — cmux で別ペインに各ロールを起動（進捗がリアルタイムで見える）"
```
