...
- 未解決の **Minor** findings を列挙（Minor は記録するが修正しない — REQ-007）。
...
最終 `evaluate-{n}.md` と `evidence/{n}/` ディレクトリから:

- 要件ID別の合否表（項目・要件・検証方法・判定）。
- 各行の証跡を `.specs/{feature}/evidence/` 配下の **相対パス** で示す。レビュアーが
  PR からスクリーンショットやログを開けるように。
...
**Critical / Improvement** findings を列挙し、draft を引き取る人に残作業を正確に
伝える。
...
| 裁定の draft 着地（停滞） | **draft** | 上記 + `## Unresolved`（Critical / Improvement） |
