...
1. spec-generator を planner として起動（`spec_author` バックエンド）。
   ...
   - `spec_author` と `host_runtime` が異なる場合、agent-delegate backend は初回生成も
     修正もファイルを書き込む。author role を target として明示し、`--detach` を渡して
     expected run id を保持し、`../role-dispatch.ja.md` の15〜30秒 expected-run 待機を使う。
     呼び出し側のタイムアウトは20分以上とする。
