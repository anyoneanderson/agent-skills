...
5. **メトリクス行を最後に追記する。** 適用ステップの後、`.specs/pipeline-metrics.jsonl`
   に1行追記し、`applied_improvements` を実際に適用した内容にする（何も適用しない・
   縮退・pr 未到達なら `[]`）。JSON Lines は追記専用で後から埋められないため、追記を
   ここまで遅延させる。
...
- retrospective の結果（レポートパス、メトリクス行、適用した改善 or 起票 Issue）を
  `state` に記録し、`completed_phases` に `retrospective` を追加する。
