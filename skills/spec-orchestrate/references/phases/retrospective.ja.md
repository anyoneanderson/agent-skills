...
判定と自動適用 / revert の仕組みは improve-apply 規則（T016）で、参照はするがここでは
実装しない。
...
比較が成立しないため T016 の適用ステップをスキップ）。既存の state ファイルに対して
このフェーズを単体実行することもできる。
...
3. **メトリクス比較。** 前回実行の行を読み、共有メトリクス（ラウンド、blocker
   ...
   revert の判断は improve-apply 規則（T016）。retrospective は比較を記録するだけ。
4. **改善の適用**（Tier 判定・行数バジェット検査・ブランチ/PR/マージ または Issue 起票
   への縮退・revert）は improve-apply ステップ（T016）。pr 未到達の実行ではスキップ。
...
- （T016 経由）改善ブランチ/PR または起票した Issue。
...
- レポート + メトリクス記入（+ T016 経由の改善 or Issue 縮退）完了 → **（終了）**
