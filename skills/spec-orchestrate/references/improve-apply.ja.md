...
- **`improve.skills_repo` 未設定 or 書き込み不可** → Issue 起票のみに縮退（下記
  ...
  `improve.skills_repo`）、インストール済みコピーには適用しない。
- **`improve.auto_apply: false`** → Tier 1 の提案も PR を人間レビュー待ちで残し、
  自動マージはしない。

## fresh metricsと外部actionの冪等性

比較とrevertの入力は`references/scripts/retrospective-ledger.sh list-active`だけから読む。
生のJSONL順序は有効なselectorではない。後ろの行がsupersede eventの場合があり、古い
metrics rowは中間terminal stateを表す可能性がある。selectorはsupersede済みrecordを除外し、
同じ`run_id`にactive recordが複数あれば失敗する。

改善PR、revert PR、fallback Issueを作る前に、
`action_key = sha256(proposal_id + action_kind + normalized_target + change_digest)`を算出し、
`status: "pending"`として`state.retrospective.action_history`へ予約する。keyが既にあれば
別actionを作らない。pending entryは、本文またはbranch metadataに同じkeyを持つ外部itemを
検索して突合する。見つかればURL/番号と`completed`を記録し、作成されていないと確認できた
場合だけ再試行する。完了済みretrospectiveを失効・再生成してもaction historyを保持する。
...
ため、自動書き換えはそれらを壊しうる。SKILL.md と scripts はスキルの実行面
...
*プロジェクト* を cwd に実行し `skills_repo` は別ディレクトリのため、cwd に
アンカーすると正当な適用先も (2) の repo-root 配下チェックで全拒否される
（fail-closed なのでセキュリティ穴ではないが、自動適用が機能しない）。
...
が行う（オーケストレーターはスキルファイルを自分で編集しない — 委譲のみ）。

1. `improve.skills_repo` で改善ブランチ `improve/{feature}-{run-id}` を作る。
2. stateへaction keyを予約し、PR本文へhidden markerとして含める。ワーカーの
   サブエージェントがそのブランチで提案の編集を適用する。
...
改悪は `git revert` 一発で戻せる。

## 縮退（書き込み可能な skills リポジトリがない）

`improve.skills_repo` 未設定 or 書き込み不可なら、何も適用しない。代わりに提案（対象・
Tier・根拠）とaction-key markerを含むIssueをagent-skillsリポジトリに起票する。改善PRと
同じ規則でkeyを予約・突合する。書き込み権限なしでも学びを残し、terminal rerunでIssueを
二重起票しない。
...
退行シグナルは`pipeline-metrics.jsonl`から選んだactive recordだけから読む
（`retrospective-format.ja.md`の
