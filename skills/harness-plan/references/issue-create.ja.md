...
### Sprint-0 (type: foundation) body テンプレート

foundation-sprint は通常 sprint と異なる body 構造。acceptance scenario /
rubric は持たず、deliverables チェックリスト + 外部設定指示で構成する。
contract の完全な semantics は
[foundation-sprint-guide.ja.md](foundation-sprint-guide.ja.md) 参照。
...
**Type**: `foundation`（greenfield bootstrap — attestation 後に rubric 採点 sprint）
**Generator mode**: `<none | scaffold | optional>`
**Dependencies**: —

## この sprint が存在する理由

`/harness-plan` Step 3.5 が foundation readiness を `<YELLOW|RED>` と
判定した。この sprint を済ませないと後続の feature sprint で
harness-loop が実行不能。**rubric 採点せず human attestation で完了**。

## Deliverables

<contract の deliverable キーをチェックボックスで列挙。各項目は
foundation-readiness.sh --check <key> で ok を返す必要がある。>
...
- [ ] external_setup_doc
- [ ] dev_db_available
- [ ] (Planner が追加した独自キーがあればここに)

## External setup（人間のみ、Generator 対象外）

<Planner が contract.md の Setup Prerequisites 節から埋める。
product-spec.md Constraints を出典に:>

- [ ] GCP project + OAuth consent screen (Internal, @example.com)
- [ ] OAuth client (Web) + redirect http://localhost:3000/api/auth/callback/google
- [ ] Slack App 作成、Signing Secret 取得
- [ ] Anthropic API key を .env.local に
- [ ] docker-compose up（docker-based dev DB 使用時）

## Verification

harness-loop が各 deliverable について `foundation-readiness.sh --check <key>`
を実行し、結果を
...
sprint クローズに必須。harness-loop が AskUserQuestion で
attestation を求める → "Attest" 選択で sprint 1 negotiation に進む。

## PR

単独 PR、タイトル `[sprint-0] foundation: ${feature}`。
PR body は rubric スコアでなく deliverables と検証結果を列挙する。

🤖 Managed by `/harness` skill
```

重複検出 / rate limit 処理は feature sprint と同じ（上記参照）。

### Label 衛生 — `foundation` ラベル

roadmap に foundation-sprint が含まれる場合は、Step 8 で 3 つ目の
ラベル `foundation`（color `#5319E7`、description "Foundation sprint —
human-attested bootstrap"）を `gh label create --force` で冪等に作成。
