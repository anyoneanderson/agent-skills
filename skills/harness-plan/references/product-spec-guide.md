...
## Greenfield Exception — Extracting Constraints From Existing Docs

The "What, not How" rule above excludes framework / library / DB picks
from `Constraints` by default. This assumes the user volunteers any
external mandates during interview. In **greenfield** projects, an
architectural mandate is often already written down in
`PROJECT.md` / `README.md` / `ARCHITECTURE.md` (or equivalent) as part
of the initial brief. Those files are themselves an external source —
they pre-date the interview — so their tech-stack declarations are
legitimate `Constraints` material.

When the project contains such a document, the Planner MUST:

1. Scan it before the interview for tech-stack signals:
   - Manifest samples (`package.json` excerpts, `Cargo.toml`)
   - ORM / schema samples (`prisma/schema.prisma`, `models.py`,
     SQLAlchemy declarative base)
   - Directory structure diagrams naming runtime folders (e.g.
     `├── prisma/`, `├── docker-compose.yml`)
   - "Tech stack" tables
2. Build a list of "document-sourced Constraint candidates" (framework,
   language, package manager, ORM, DB, auth provider, hosting target)
3. Present the list to the user at interview start for per-item
   approve/reject:
   ```
   AskUserQuestion:
     question: "Found the following tech-stack mandates in PROJECT.md.
               Treat them as non-negotiable Constraints?"
     options (per candidate):
       - "Accept (Recommended)"
       - "Keep out of Constraints — let Implementation Loop decide"
   ```
4. Put approved items under `## Constraints` with the source cited:
   `- Package manager must be pnpm (PROJECT.md L261 directory layout)`
5. Reject items go into interview notes but not `product-spec.md`

**Why this matters**: these mandates drive the `foundation-readiness.sh`
probes in Step 3.5 (e.g. a "Prisma" constraint requires
`prisma/schema.prisma` to be present, or it becomes a missing probe).
Without them, the foundation-sprint deliverables cannot be derived and
sprint-0 will have an empty checklist.

**Still reject these**:

- Preferences without a citable source ("I prefer Redux") — belongs in
  Implementation Loop, not Constraints
- Architectural speculation without a mandate ("maybe Prisma") — interview
  the user to firm it up or leave it out
- Self-declared constraints that contradict the document — flag to user
  for resolution before drafting proceeds
