...
## Greenfield 例外 — 既存ドキュメントからの Constraints 抽出

上記「How を書かない」ルールは、framework / library / DB 選定を
`Constraints` から除外することを既定とする。これはユーザがインタビューで
外部強制事項を申告する前提だが、**greenfield** では多くの場合、
`PROJECT.md` / `README.md` / `ARCHITECTURE.md` 等の初期ドキュメントに
既に tech-stack が宣言されている。これらのファイル自体が外部由来ソース
（インタビュー以前に存在）なので、そこにある tech stack 宣言は正当な
`Constraints` 素材とみなしてよい。

該当ドキュメントがある場合、Planner は:

1. インタビュー前にスキャンし tech-stack signal を拾う:
   - Manifest サンプル（`package.json` 抜粋、`Cargo.toml`）
   - ORM / schema サンプル（`prisma/schema.prisma`、`models.py`、
     SQLAlchemy declarative base）
   - ディレクトリ構成図で runtime フォルダを明示するもの
     （`├── prisma/`、`├── docker-compose.yml` 等）
   - 「技術スタック」表
2. 「ドキュメント由来 Constraints 候補」リストを作る
   （framework / 言語 / パッケージマネージャ / ORM / DB / 認証プロバイダ /
   ホスティング先）
3. インタビュー開始時にユーザへ項目別に承認/却下を求める:
   ```
   AskUserQuestion:
     question: "PROJECT.md に以下の tech-stack 指定が見つかりました。
               交渉不可 Constraints として扱いますか？"
     options (候補ごと):
       - "承認 (Recommended)"
       - "Constraints から外す — Implementation Loop に判断を委ねる"
   ```
4. 承認されたものを `## Constraints` に出典付きで記載:
   `- パッケージマネージャは pnpm（PROJECT.md ディレクトリ構成 L261）`
5. 却下されたものはインタビューメモに残すが `product-spec.md` には書かない

**なぜ重要か**: これらの指定が Step 3.5 の `foundation-readiness.sh`
probe を駆動する（例: "Prisma" 制約があれば `prisma/schema.prisma` の
存在が probe の合格条件になる、無ければ missing 扱い）。これが無いと
foundation-sprint の deliverables を導出できず、sprint-0 が空の
チェックリストで生成される事故になる。

**以下は依然として却下する**:

- 出典のない好み（「Redux が好き」）→ Implementation Loop が決める事項
- 強制されない憶測（「Prisma 入れるかも」）→ インタビューで固めるか削除
- ドキュメントと矛盾する自己申告 → ユーザに確認して下書き開始前に解決
