...
generator_backend: <roadmap.md sprints[{{SPRINT_NUMBER}}].generator_backend をそのままコピー。legacy bypass 時は null>
generator_backend_reason: <roadmap.md sprints[{{SPRINT_NUMBER}}].generator_backend_reason をそのままコピー。null 可>
goal: {{SPRINT_GOAL}}
...
## roadmap からの backend コピー

`roadmap.md sprints[{{SPRINT_NUMBER}}]` の `generator_backend` と
`generator_backend_reason` を **そのまま** contract frontmatter にコピーする。
ここで **再判定しない** — Negotiation 段階で値が交渉される可能性はあるが、
contract draft は roadmap の確定値（または null）を反映するだけにする。

roadmap 値が `null` の場合（legacy bypass:
`_config.yml.sprint_level_generator_override == false`、または未確定）、
contract field も `null` のまま。runtime で `harness-loop` Step 1 が
`_config.yml.generator_backend` に fallback する。
...
- **`generator_backend` の再判定**（roadmap.md からそのままコピー）
