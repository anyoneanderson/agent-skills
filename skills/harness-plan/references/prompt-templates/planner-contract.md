...
generator_backend: <copy verbatim from roadmap.md sprints[{{SPRINT_NUMBER}}].generator_backend; may be null when legacy bypass is on>
generator_backend_reason: <copy verbatim from roadmap.md sprints[{{SPRINT_NUMBER}}].generator_backend_reason; may be null>
goal: {{SPRINT_GOAL}}
...
## Backend copy from roadmap

Copy `generator_backend` and `generator_backend_reason` **verbatim** from
`roadmap.md sprints[{{SPRINT_NUMBER}}]` into the contract frontmatter
above. Do **not** re-judge — Negotiation may negotiate the value later,
but the contract draft must reflect the roadmap's confirmed (or null)
value as-is.

If the roadmap value is `null` (legacy bypass:
`_config.yml.sprint_level_generator_override == false`, or the field was
never written), the contract field is also `null`. At runtime,
`harness-loop` Step 1 falls back to `_config.yml.generator_backend`.
...
- **Re-judge `generator_backend`** — copy verbatim from roadmap.md
