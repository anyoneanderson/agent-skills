...
### Sprint-0 (type: foundation) body template

foundation-sprints use a different body structure — no acceptance scenarios,
no rubric, checklist + external-setup instructions instead. See
[foundation-sprint-guide.md](foundation-sprint-guide.md) for the full
contract semantics.

```bash
gh issue create \
  --title "[sprint-0] foundation: ${feature}" \
  --body-file <(generate_foundation_body) \
  --label "harness,sprint,foundation" \
  --assignee "@me"
```

`generate_foundation_body()` output:

```markdown
Parent epic: #<EPIC_NUMBER>

## Sprint 0 — ${feature} (foundation-sprint)

**Type**: `foundation` (greenfield bootstrap — rubric-scored sprint
after attestation)
**Generator mode**: `<none | scaffold | optional>`
**Dependencies**: —

## Why this sprint exists

Step 3.5 of /harness-plan reported `<YELLOW|RED>` on foundation
readiness. This sprint makes harness-loop feasible for the feature
sprints that follow. It is **human-attested**, not rubric-scored.

## Deliverables

<One checkbox per deliverable key from the contract. Each must pass
foundation-readiness.sh --check <key> before human attestation.>

- [ ] package_manifest
- [ ] runtime_boots
- [ ] test_runner_configured
- [ ] env_example_committed
- [ ] external_setup_doc
- [ ] dev_db_available
- [ ] (custom keys added by Planner, if any)

## External setup (human-only, no Generator)

<Planner fills from contract.md's Setup Prerequisites section, sourced
from product-spec.md Constraints. Examples:>

- [ ] GCP project + OAuth consent screen (Internal, @example.com)
- [ ] OAuth client (Web) with redirect http://localhost:3000/api/auth/callback/google
- [ ] Slack App created, Signing Secret copied
- [ ] Anthropic API key in .env.local
- [ ] docker-compose up (if docker-based dev DB)

## Verification

harness-loop runs `foundation-readiness.sh --check <key>` for each
deliverable and reports pass/fail in
`.harness/${EPIC}/sprints/sprint-0-${feature}/feedback/verification-1.md`.

## Human attestation

Required before the sprint closes. harness-loop surfaces an
AskUserQuestion; choose "Attest" to advance to sprint 1 negotiation.

## PR

Single PR for this sprint, title `[sprint-0] foundation: ${feature}`.
The PR body lists deliverables + verification results, not rubric scores.

🤖 Managed by `/harness` skill
```

Duplicate detection and rate limit handling work the same way as feature
sprints (see above).

### Label hygiene — `foundation` label

Add a third label `foundation` (color `#5319E7`, description "Foundation
sprint — human-attested bootstrap") idempotently via `gh label create
--force` at Step 8 when a foundation-sprint exists in the roadmap.
