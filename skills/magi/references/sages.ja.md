...
**必要な最低バージョンは codex-cli 0.145.0** で、`mcp list --json` と `--disable`
を検証したのがこのリリースである。これより古い codex は未知のフラグを拒否するため、
ラッパーは後述のとおり停止し、BALTHASAR は「回答なし」として記録される。合議は
2体へ縮退するが、問いが漏れることはない。BALTHASAR が毎回失敗するときは、まず
`codex --version` を確認すること。

このラッパーは、次の2つの場合に**フェイルクローズで止まる。** どちらも「遮断すべき
サーバーの集合が分からない」状態である。

- 一覧の呼び出しが失敗した場合、または `name` を持つオブジェクトの配列以外が返って
  きた場合: exit 2。codex 自身のメッセージは、停止の通知と並んで stderr に残る。
- codex が `[A-Za-z0-9_-]` だけで書かれていない名前のサーバーを報告した場合:
  exit 3。ドット記法の `-c` パスに変換できないためである。そのサーバーを改名するか
  削除すること。

いずれの場合も BALTHASAR は「回答なし（エラー）」として現れ、理由は
...
**ここでの 0.145.0 は「たまたま検証したバージョン」ではなく必要な最低条件である。**
同梱の `codex-sage.sh` が `mcp list --json` と `--disable` を必要とし、それが無い
環境では実行を続けずに停止する。
