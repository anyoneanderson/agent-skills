...
options:
  - name: "Playwright (a11y snapshot) (Recommended for web) / Playwright（a11y スナップショット）（Web 推奨）"
    description: "ブラウザを自動操作して UI をアクセシビリティツリー（DOM の構造的スナップショット）で検証。flaky な画面キャプチャ比較を避ける"
  - name: "pytest / pytest"
    description: "Python ベースのテストランナー。任意の HTTP/API 系バックエンドサービスのテストに最適"
  - name: "curl / curl"
  ...
  - key: allowed_mcp_servers
    ...
    options:
      ...
      - name: "独自リスト / Custom list"
        description: "カンマ区切りで指定（例: `playwright, github-zenchaine, context7`）"
```
