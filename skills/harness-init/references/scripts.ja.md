...
| `check-sprint-prs-complete.sh` | retroactive check | completed/done epic に必要な `sprint_prs[n]` が欠けていたら失敗 |
| `wrap-untrusted.sh` | Orchestrator 補助 | 外部コンテンツを `<untrusted-content>` で包む |
| `foundation-readiness.sh` | `/harness-plan` Step 3.5 + `/harness-loop` foundation 検証 | greenfield signal を probe し severity JSON と epic 単位レポートを出力 |
...
|---|---|---|---|
| active epic なし | `current_epic` | — | null |
| Non-loop phase | `phase` ∈ allowlist | — | 下記参照 |
...
**Non-loop phase allowlist** — 以下の phase では Principal Skinner 判定を完全に
スキップし常に allow stop を返す。自律 sprint loop が走っていない（または終了
した）ため iteration / wall / cost 上限は意味を持たない:

- `product-spec-draft` / `roadmap-draft` / `roadmap-approved` / `issues-pending`
  — `/harness-plan` の対話 checkpoint
- `ready-for-loop` — plan 完了後、`/harness-loop` 起動前のハンドオフ状態
- `done` — 最終 sprint 終了後の terminal state
- `foundation-setup` / `foundation-attest` — foundation-sprint の human-driven
  フェーズ（rubric scoring 無し、G⇄E loop 無しなので iteration 等は無意味。
  foundation-sprint-guide.ja.md 参照）

実効制御は通常（非 foundation）sprint の `negotiation` / `impl` / `evaluation`
/ `pr` phase のみで働く。
