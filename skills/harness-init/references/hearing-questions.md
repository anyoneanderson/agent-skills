...
- If `codex --version` fails → remove `codex_cli` and `codex_cmux`
  options; advise the user to run
...
options:
  - name: "Codex CLI (Recommended) / Codex CLI（推奨）"
    description: "Run Codex (gpt-5.4) as the Generator via `codex exec --sandbox danger-full-access`. Plugin-free, deterministic, and resilient to foundation-sprint network / cache constraints. The Orchestrator canonicalises the final message and Git diff through the dispatch script."
  ...
  - name: "Codex via cmux (visibility mode) / Codex（cmux 経由、可視モード）"
    description: "Same role contract as Codex CLI, but Codex runs inside a cmux-delegated pane you can watch. Useful for long-running sprints where you want to see Codex reason in real time, or for debugging Codex behavior. Requires Codex CLI + cmux-delegate skill."
...
(multiSelect=true; choose one or more tools in primary to fallback order)

options:
  - name: "Playwright MCP (a11y snapshot, live user simulation) (Recommended for web) / Playwright MCP（a11y スナップショット、ライブ操作）（Web 推奨）"
    description: "Evaluator uses `mcp__playwright__browser_*` tools to operate the dev server like a user. Best when Playwright MCP is installed and the sprint needs live browser verification."
  - name: "Playwright CLI (independent spec, no stubs) / Playwright CLI（独立 spec、stub 禁止）"
    description: "Evaluator writes its own `.spec.ts` under `evidence/evaluator-tests/`, executes it without `page.route` / `addInitScript` / `window.fetch` overrides, and keeps it as a regression asset."
  - name: "Cypress / Cypress"
    description: "Evaluator writes browser runtime checks with the project's Cypress setup when that is the established E2E tool."
  - name: "curl / curl"
    description: "Evaluator verifies contract boundaries with raw HTTP calls and shell scripts. Best for API-only projects where browser automation adds little value."
  - name: "Custom script / 独自スクリプト"
    description: "Evaluator authors `.harness/scripts/eval-<feature>.sh` when the project needs a custom verification harness beyond the built-in tool references."
```

**Config key**: `evaluator_tools` (list)
**Constraint**: Store an ordered list. For web projects, recommend
`[playwright-mcp, playwright-cli]` as primary + fallback; API projects
usually use `[curl, custom-script]`, and CLI projects usually use
`[custom-script]`. Allowed values are `playwright-mcp`,
`playwright-cli`, `cypress`, `curl`, or `custom-script`.

**Render note**: Step 2 writes the selected order into `_config.yml`.
An empty answer renders `evaluator_tools: []`; project-specific defaults
belong in the deployed config, not in templates.
  ...
  - key: allowed_mcp_servers
    ...
    options:
      ...
      - name: "Custom list / 独自リスト"
        description: "Specify comma-separated server names (e.g. `playwright, github, context7`)."
```
