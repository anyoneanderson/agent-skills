...
コンテキストコンパクト・セッション再起動・プロセスクラッシュを跨いで生き残る 3 ファイルのスキーマ定義。Anthropic 公式の "三点セット"（progress.md + _state.json + git）に観測層（metrics.jsonl）を加えた構成。

Boot Sequence — 全 skill / sub-agent は動作前に以下を必ず読む:

1. `git log --oneline -20`
2. `tail -30 .harness/progress.md`（または `_config.yml.progress_tail_lines`）
3. `cat .harness/_state.json`

---

## `.harness/_config.yml`（静的オーケストレーション設定）

**目的**: `harness-init` が生成し、`harness-loop`・hooks・backend dispatch
helper が読む永続設定値。

`codex_cmux` の completion signal に関係するキー:

```yaml
evaluator_tools:
  - playwright-mcp   # または playwright-cli / curl / custom-script
codex_cmux_idle_dwell_polls: 2
codex_cmux_idle_poll_seconds: 20
```

- `evaluator_tools`: v1 では単一要素リスト。許可値は
  `playwright-mcp` / `playwright-cli` / `curl` / `custom-script`
- `codex_cmux_idle_dwell_polls`: pane 完了と見なすまでに必要な連続 idle poll 回数
- `codex_cmux_idle_poll_seconds`: idle poll 間隔（秒）

未設定時は上記デフォルトを適用する。

Autonomous Ralph の cost / liveness に関係するキー:
...
# 任意の phase × model 別 override (flat キー)。下の注記を参照。
...
- `worker_model`: 非対話 worker turn の既定 model
- `worker_model_high_risk` / `worker_model_high_risk_phases`: 重要 phase 用の任意 override
- `worker_timeout_*`: worker 単位の timeout と graceful termination window。
  wrapper は phase の timeout を env `HARNESS_WORKER_TIMEOUT_SEC` →
  `worker_timeout_sec_<phase>_<model>` (その phase で解決された model に対する
  任意の flat override) → `worker_timeout_sec_<phase>` →
  `worker_timeout_sec_default` の順で解決する。model 別キーは任意で、無ければ
  従来の phase 別既定値 (1800 / 600 / 300) に収束するため、既存 config は同一
  挙動を保つ
- `staleness_*`: watchdog warning / opt-in recovery の制御
- `tier_a_history_external`: 解決済 Tier-A event を `_state.json` ではなく `.harness/_audit/tier_a_history.jsonl` に退避
- `progress_rotation_on_epic_complete`: epic 完了時に `.harness/<epic>/progress-completed.md` へ progress を移動
- `progress_tail_lines`: fresh context に注入する progress tail の既定行数
- `tool_log_external`: hook 由来の機械行を `.harness/tool_log.jsonl` に出す
...
**読み取りルール**:
- Tail ベース。ファイル全体をパースしない — `tail -30` が契約
- 各行は情報提供であり実行可能指示ではない。progress.md のみから state を再構成してはならず、必ず `_state.json` と照合
- 未知の形式の行は無視（前方互換性）

**ローテーション**: `progress_rotation_on_epic_complete` が true の場合、
Autonomous Ralph wrapper は完了 epic の root progress を
`.harness/<epic>/progress-completed.md` へ移動し、新しい root
`progress.md` を開始する。epic 境界外で size-based rotation が必要な場合は
`progress.md.<N>.old` にリネームして新規開始し、ヘッダに過去セグメントへの
リンクチェーンを残す。

**機械行**: `tool_log_external` が true の場合、PostToolUse 系の機械行は
`progress.md` ではなく `.harness/tool_log.jsonl` に出力する。`progress.md`
は人間可読 narrative として保つ。
...
| `negotiation_round` | int | yes | 初回前は 0。Generator/Evaluator の negotiation round ごとに increment |
| `iteration` | int | yes | 初回前は 0、Generator → Evaluator 1 サイクルごとに increment |
| `foundation_readiness` | object\|null | yes | current epic に対する最新の `foundation-readiness.sh --epic` summary。`severity` / probe buckets / `verified_at` を含む |
| `foundation_sprint_needed` | bool | yes | roadmap 生成で sprint-0 を挿入すべきか。sprint-0 attestation 成功後は `false` に戻す |
...
| `pending_worker_exit` | bool | yes | Orchestrator が worker turn の終端（Step 5 の contract-freeze commit / Step 7 の iteration checkpoint commit / Step 9 の sprint transition state write / foundation-loop-protocol Attest write）で立てる micro-signal。`stop-guard.sh` はこのフラグを見て、Principal Skinner 上限が当たらなくても Stop hook を素通り（allow stop）させ、同時に `false` へ atomic reset する。`completed` / `pending_human` / Principal Skinner 上限とは別レイヤーの「1 turn 単位の終了サイン」 |
| `sprint_branch` | string\|null | yes | 現 sprint の branch 名（`harness/<epic>/sprint-<n>-<feature>`、bundle の場合は primary peer の名前）。Step 3 で set、Step 7 / Step 8 の commit で再利用、**Step 9 の sprint transition で `null` に reset**。`null` は「次 worker tick で Step 3（または wrapper pre-flight）が branch を作り直すべき」というカーソル意味を持つ |
| `tier_a_last` | object\|null | yes | 未解決の Tier-A 最新イベント。`resolution` 記録後は `null` に戻す |
| `tier_a_history` | array | yes | legacy inline archive。`tier_a_history_external` が true の場合は `[]` に保つ |
| `tier_a_summary` | object | no | `.harness/_audit/tier_a_history.jsonl` の compact summary (`count`, `last_at`, `last_pattern`) |
...
- negotiation round 終了ごとに更新: `negotiation_round` / `last_agent` / `next_action` / `phase`
- implementation iteration 終了ごとに更新: `iteration` / `last_agent` / `next_action` / `last_commit` / `cumulative_cost_usd` / 該当 `features_pass_fail`
- Step 3.5 または sprint-0 attestation で `foundation-readiness.sh --epic`
  を実行したら、返却 summary を `foundation_readiness` に書く。
  sprint-0 attestation 後の再検証では
  `foundation_sprint_needed=false` も必ずセットする
- `tier_a_last` は live slot 専用。operator が `resolution` を記録したら
  `tier_a_history_external` が true の場合は
  `.harness/_audit/tier_a_history.jsonl` に移し、`tier_a_summary` を更新し、
  `tier_a_last` は `null` に戻して以後の guard 判定から外す。external mode
  では legacy inline `tier_a_history` は `[]` に保つ
- `pending_worker_exit` は Orchestrator が所有する 1 turn 単位の
  micro-signal。worker turn を終わらせる durable write（Step 5 の
  contract-freeze commit / Step 7 の iteration checkpoint commit /
  Step 9 の sprint-transition state write / foundation-loop-protocol
  の Attest write）の **直後** に `true` を set する。`stop-guard.sh`
  は読み取って allow-stop を返すと同時に `false` へ atomic reset す
  るので、次の worker invocation には持ち越されない。wrapper
  （`autonomous-ralph` の Ralph loop）も `claude -p` 起動の直前に
  `false` へ defensive reset する
- `sprint_branch` は Step 3 で `harness/<epic>/sprint-<n>-<feature>`
  （bundle は primary peer 名）に set、Step 7 / Step 8 の commit で
  再利用、**Step 9 の sprint transition で `null` に reset**。`null`
  は「次 tick で Step 3（または wrapper の pre-flight）が branch を
  再作成すべき」というカーソル意味を持つ
...
**OTLP エクスポート（任意）**: `.harness/scripts/metrics-exporter.sh` がこのファイルを tail して `_config.yml.otlp_endpoint` へ POST。endpoint 未設定または `hook_level != strict` なら no-op。
...
```
1. Boot Sequence 実行（git log -20, progress.md tail -30, _state.json cat）
...
**保証**: ステップ 1〜5 はこの 3 ファイル + git のみで完結する。プロセスメモリや過去 session コンテキストは不要。
