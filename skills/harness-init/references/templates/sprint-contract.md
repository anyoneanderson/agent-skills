<!--
  ...
  RULES:
    ...
    - `generator_backend` is negotiable during Negotiation rounds (alongside
      max_iterations). Mid-impl change requires Planner ruling. Setting it
      to null defers to roadmap.md / _config.yml fallback.
-->
...
generator_backend:                   # claude | codex_cli | codex_cmux | null (= roadmap / _config.yml fallback)
generator_backend_reason:            # backend を明示する場合は根拠を記述（例: "UI / UX 重視 sprint。roadmap 推奨に従う。"）
goal: |
