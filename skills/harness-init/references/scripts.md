...
| `codex-cli-dispatch.sh` | Orchestrator helper | Run `codex exec`, canonicalise outputs, and bridge to harness state |
| `check-sprint-prs-complete.sh` | Retroactive check | Fail if a completed/done epic is missing any required `sprint_prs[n]` |
| `wrap-untrusted.sh` | Orchestrator helper (not a hook) | Wrap external content in `<untrusted-content>` |
| `foundation-readiness.sh` | `/harness-plan` Step 3.5 + `/harness-loop` foundation verifier | Probe greenfield signals; emits severity JSON and per-epic report |
...
|---|---|---|---|
| No active epic | `current_epic` | — | null |
| Non-loop phase | `phase` ∈ allowlist | — | see below |
...
**Non-loop phase allowlist** — these phases skip Principal Skinner enforcement
entirely because no autonomous sprint is running yet (or anymore):

- `product-spec-draft` / `roadmap-draft` / `roadmap-approved` / `issues-pending`
  — `/harness-plan` interactive checkpoints
- `ready-for-loop` — post-plan handoff state before `/harness-loop` starts
- `done` — terminal state after the last sprint finishes
- `foundation-setup` / `foundation-attest` — human-driven foundation-sprint
  phases; iteration / wall / cost caps are meaningless because there is no
  rubric-scored G⇄E loop on a foundation-sprint (see foundation-sprint-guide.md)

Active enforcement only applies during `negotiation` / `impl` / `evaluation`
/ `pr` phases of a normal (non-foundation) sprint.
