...
- **Evidence Manifest** は結果が引く各証跡ファイルを、バイトサイズと sha256 ダイ
  ジェストとともに列挙する。証跡ファイル自体（運転記録。ローカルに残す）をコミット
  せずに、検証可能な証跡の記録を PR に載せるためにある。各行は `shasum` で計算する:

  ```bash
  # 証跡ディレクトリから。wc -c がバイトサイズ、shasum -a 256 がダイジェスト
...
3. Evidence Manifest の各行について、実ファイルの sha256 とバイトサイズを再計算し
   マニフェストと照合する。ダイジェストが一致しない（ディスク上の証跡がマニフェスト
   の記録と異なる）場合、その項目の判定を FAIL に書き換え、Critical finding を追加
   する: 「evidence hash mismatch for {case}: {file}」。
4. `## Summary` の集計と `Gate:` 行を再計算する。

これが証拠優先の原則の裏づけ: 証跡のない自己申告の合格は、受理された結果まで生き残れ
ない。

**期待値が空出力のケースは非空のラッパーログを証跡にする。** ケースの期待結果が
空出力（例: 何も出力しないはずのコマンド、空 diff の検証）のとき、生の空ファイルを
証跡に指さない — 機械検査は空の証跡ファイルを欠落扱いとし、正当な PASS を FAIL に
反転させる。代わりに非空のラッパーログを証跡として書く: 実行したコマンド・終了
コード・出力が期待どおり空だった旨の明示行（例: `stdout: <empty> (0 bytes) —
expected`）を記録する。ラッパーは非空なので検証を通過しつつ、空という結果を忠実に
捉える。
