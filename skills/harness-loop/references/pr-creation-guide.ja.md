...
1. `contract.status == "done"`（最終 iteration で全 rubric 軸が threshold 以上。
   `type: foundation` は human attestation 完了が条件）
2. `_state.json.aborted_reason == null`
3. sprint ブランチがローカルに存在し、親ブランチより 1 commit 以上進んでいる。
   **ブランチ作成は SKILL.md Step 3「Sprint branch setup」が担当**する。
   本ガイドに到達時点で `_state.json.sprint_branch` は populated、HEAD は
   その branch 上にある前提。もし HEAD が default branch 上なら loop 側の
   バグ — halt して surface する（過去の halt-on-bug 事案に基づくガード）。
...
- **Labels**: `gh label create ... || true` で `harness-loop` の存在を
  保証したうえで常に付与。sprint の `roadmap.md` に `labels:` があれば
  pass-through
