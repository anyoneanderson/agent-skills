<!--
  ...
  threshold proposals and trade-off reasoning belong to the Generator's
  judgment. The Orchestrator does not suggest specific threshold values
  or pre-compute "suggested" counter-proposals here.
-->
...
**Atomicity rule (MANDATORY)**: write the canonical negotiation feedback
pair once, as the last action of this invocation. Do NOT overwrite these
paths with intermediate drafts; keep scratch notes elsewhere.
...
- Threshold proposals must stay consistent with `docs/coding-rules.md`
  MUST rules. Do NOT propose thresholds that would require bypassing
  any MUST rule to meet, and do NOT propose a Craft threshold that
  ignores `docs/review_rules.md` Critical findings.
- Assume you must ship at least one integration-level test that crosses
  the contract boundary. Stub-only tests using `page.route`,
  `addInitScript`, `window.fetch` override, or equivalent full mocks
  will not be accepted by Evaluator as Functionality proof.
