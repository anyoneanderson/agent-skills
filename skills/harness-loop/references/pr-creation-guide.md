...
1. `contract.status == "done"` (all rubric axes ≥ threshold on the
   final iteration; for `type: foundation`, human attestation received)
2. `_state.json.aborted_reason == null`
3. The sprint branch exists locally and is ahead of the parent branch
   by at least one commit. **Branch creation is handled by SKILL.md
   Step 3 "Sprint branch setup"** — by the time this guide runs,
   `_state.json.sprint_branch` is populated and HEAD is on it. If HEAD
   is on the default branch at this point, the loop has a bug; halt
   and surface. This guards against a known halt-on-bug incident; the
   defensive checks.
...
Ensure the required label exists before creating the PR:

```bash
gh label create harness-loop \
  --color "0E8A16" \
  --description "Auto-opened by harness-loop" \
  2>/dev/null || true
```
...
- **Labels**: ensure `harness-loop` exists via `gh label create ... || true`,
  then add it always. If the feature's sprint in `roadmap.md` carries
  `labels:`, pass them through.
