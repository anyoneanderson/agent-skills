...
この文書は `references/scripts/agent-delegate.sh` の公開インターフェースです。上位
スキル（spec-orchestrate など）はこの契約に依存し、`SKILL.md` を経由せずスクリプトを
...
| `env_error` | report を生成せず終了（セーフティネットが合成）。ブロックに至る git 外実行等も含む |
| `unclassified` | パターン不一致の非0終了 |

## review モード

- 常に `read-only`。レビュワー自身はファイルを書けないため、spec-review 互換のレビュー
  ファイル全文を **最終メッセージ**として出力し、スクリプトがそれを `--review-output` に書き出す。
...
- 通過したレビューファイルは `spec-code --feedback` の入力としてそのまま使える。
