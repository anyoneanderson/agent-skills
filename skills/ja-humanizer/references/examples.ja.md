...
出典: この スキル自身の紹介記事を ja-humanizer で書かせた Zenn 版（ですます）の冒頭。書き手が「妥協せずに文体まで」直した。検査スクリプトは before も after も全等級0件で、差分は機械で数えられる癖の外側にある。
...
期待する検出: before も after も第1級0件。この組は検出器の検査ではなく、見本として読ませて書き直しの方向を確かめるために使う。
