...
## 読み物モードの添削（Zenn 版紹介記事、書き手が文体まで直した）

出典: この スキル自身の紹介記事を ja-humanizer で書かせた Zenn 版（ですます）の全文。書き手が「妥協せずに文体まで」直し、等級の説明はスキルが加筆したものを書き手がさらに直した。検査スクリプトは before も after も全等級0件で、差分は機械で数えられる癖の外側にある。
...
- 断定を傾向の形にする。「目印です」「揃います」を「多用しがちです」「傾向があります」「と考えています」に。
- 一人称と内心の引用で始める。「私も」「自分でも」「あ、AI に書かせたな」。抽象的な導入を、自分が何を見てどう感じたかに置き換える。
- 決め台詞と一文の答えを捨てる。「語のリストが英語だからです」「足したのは事実だけです」を、理由を連ねた説明文に戻す。
- 他人の仕事には敬語と、参考にした事実を添える。「作っておられる人もおられたので参考にさせていただきました」。上からの評価語（「よく捉えています」）は外す。
- 比喩を動作に戻す。「空振りします」を「あまり意味をなさないので採用していません」に。
- 脇道と留保を入れる。「まあコンテンツまで全部 AI 任せでは自分の記事では無くなってしまうので、賛否両論ありますが」「当たり前ですが」。
- 見出しは名詞句にする。「なぜか？」を「理由」に。
- 具体で地面に付ける。「Claude 系が特に顕著」「過去の記事で決済サービスの導入メリットを書かせた時の事例」。

- 仕組みの説明に戻す。「補うと捏造になるからです」の一文の答えを消し、「〜ようになっています」「書き手が回答をしてくれたらその内容を元に書き直します」と、何が起きるかを順に書く。
- 設計の理由を自分の価値判断として一文添える。「AI が知り得ない事実や補強すべきコンテンツを与えるのは書き手のオリジナリティが出る部分だと思うので、そこは大事にしました」。
- 読者への指示は直接書く。「導入は1行です」を「導入は以下をコマンドから打ってください」に。
- 断定を一段下げる細部。「必ず直したい」を「まず直したい」に、「壊れる箇所」を「合わなくなる箇所」に、「やめました」を「断念しました」に。一般論には「（特に私）」の括弧で自分を入れる。

期待する検出: after は第1級0件。before は「空振り」と「壊れる」で第1級3件。この組は検出器の検査ではなく、見本として読ませて書き直しの方向を確かめるために使う。
