...
## Narrative-Mode Hand Edit (the Zenn intro of this skill's own article)

Source: the whole Zenn version (polite style) of an article about this skill, drafted with ja-humanizer, then rewritten by the writer "without compromise, down to the style"; the paragraph introducing the tiers was drafted by the skill and edited again by the writer. The checker reports zero at every tier for both versions; the difference lies beyond countable tells. 
...
ここはいきなり等級が出てくるので、等級という考え方は何かをまずは記載してください。
...
### Rules extracted from the diff

- Turn assertions into tendencies: 「目印です」「揃います」 become 「多用しがちです」「傾向があります」「と考えています」.
- Open in first person with an inner quotation: 「私も」「自分でも」「あ、AI に書かせたな」. Replace the abstract opener with what the writer saw and felt.
- Drop punchlines and one-line answers: 「語のリストが英語だからです」「足したのは事実だけです」 return to explanations that chain the reasons.
- Treat other people's work with respect and state the fact of borrowing: 「作っておられる人もおられたので参考にさせていただきました」. Remove evaluation from above (「よく捉えています」).
- Return metaphors to actions: 「空振りします」 becomes 「あまり意味をなさないので採用していません」.
- Add asides and reservations: 「まあコンテンツまで全部 AI 任せでは自分の記事では無くなってしまうので、賛否両論ありますが」「当たり前ですが」.
- Headings as noun phrases: 「なぜか？」 becomes 「理由」.
- Ground in specifics: 「Claude 系が特に顕著」「過去の記事で決済サービスの導入メリットを書かせた時の事例」.

- Return to describing the mechanism: the one-line answer 「補うと捏造になるからです」 is removed in favor of what happens, step by step (「〜ようになっています」「書き手が回答をしてくれたらその内容を元に書き直します」).
- Add the design rationale as a personal value judgment in one sentence: 「AI が知り得ない事実や補強すべきコンテンツを与えるのは書き手のオリジナリティが出る部分だと思うので、そこは大事にしました」.
- Address the reader directly: 「導入は1行です」 becomes 「導入は以下をコマンドから打ってください」.
- Small steps down from assertion: 「必ず直したい」 to 「まず直したい」, 「壊れる箇所」 to 「合わなくなる箇所」, 「やめました」 to 「断念しました」; a parenthetical 「（特に私）」 puts the writer inside a generalization.

Expected detections: zero Tier 1 in the after text; the before text has three Tier 1 hits on 「空振り」 and 「壊れる」. This pair is not a detector test; it is read as a sample to check the direction of a rewrite.
