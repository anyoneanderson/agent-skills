...
10. **spec-implement** orchestrates the full pipeline (does NOT write code or review itself):
    ...
    - Optional: `--roles` selects the Claude or Codex AI role per `kind:` label; `--host-runtime` then keeps a matching role runtime-native and sends only the opposite role through agent-delegate with an explicit target. The reviewer AI role is always the opposite of the implementer
