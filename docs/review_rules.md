...
- `.specs/**` — 仕様書ファイル
- `LICENSE` — ライセンスファイル
- `*.ja.md` — 日本語版ファイル（英語版に準拠するため、英語版のみレビュー対象）
- `.git/**` — Git 内部ファイル
...
### 重大度判定
- **重大（セキュリティ・バグ・ドキュメント更新漏れ）**: 人間の優先度は最上位
- **改善提案（品質・可読性）**: 対処を推奨
- **軽微（スタイル等）**: 対処は任意
- 重大 / 改善提案の各指摘には `fix_before`（`implementation | trial | required_check | follow_up`、既定は `follow_up` — 定義と格上げの立証責任は spec-review SKILL.md Step 4.5）を付ける。ゲートで止まるのは `fix_before: implementation` のみ。同一変更に属するドキュメント更新漏れは着地前に直すべきものなので `implementation` を付ける

### 用途別の出力
- **CI レビュー（GitHub Actions）**: インラインは高確信度のみ（QUICK: 最大5件、DEEP: 最大10件）。行特定が曖昧ならまとめコメントへ
- **レビューゲート（spec-implement）**: `fix_before: implementation` の指摘を検出し、修正ループで解消する。先送りの指摘は記録して PR 本文 / 後続 issue へ。投稿ではなく自己修正
...
ドキュメント更新漏れは**重大（Critical）**かつ `fix_before: implementation` の指摘として扱う。レビューゲートを通過するにはドキュメントの更新が必須。
